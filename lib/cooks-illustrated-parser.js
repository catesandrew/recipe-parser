...
var splitOnRegex = function(text, regex) {
  var filteredData = text,
      matches = text.match(/\(([^)]*)\)/g);

  if (matches) {
    filteredData = text.replace(/\(([^)]*)\)/g, '-placeholder-');
  }
...
var getDirectionsAndAlts = parser.getDirectionsAndAlts = function(text) {
  ...
  for (var i = 0, l = descriptions.length; i < l; i++) {
    ...
    if (regexIndexOf(desc, /^(?:(black|white)\s+pepper)$/, 0) >= 0 && tokens) {
...
var getDescriptions = parser.getDescriptions = function(text) {
  ...
  matches = splitOnRegex(description, commaSplitterRe);
  ...
  descriptions = postProcessSplitOnAnd(splitOnRegex(description, andSplitterRe)); // first try `and`
  if (descriptions.length < 2) {
...
// 'medium new potatoes boiled until tender and sliced thick'
//separated and pressed through a fine sieve
var endsWithCutIntoWedges = /(\s+cut\s+into\s+wedges)$/;
var endsWithForXRe = /(\s+for\s+(?:(dusting|drizzling|shaving)))$/;
...
var separatedAndPressedRe = /(\s+separated\s+and\s+pressed.*)$/;
var endsWithList = [
  {
    re: endsWithForXRe
  },
...
var beginsWithSkinChickenBreasts = /^((?:skinless|skin-on)\s+chicken\s+breasts)(.*)$/;
...
var adjustDescriptionsDirections = function(quantity, measurementObj, descriptionObjs, descriptions, directions) {
  ...
  var retval = _.map(_.zip(descriptions, directions), function(tuple) { //(value, key, list)
    ...
    if (_.indexOf(['boneless', 'bone-in'], zipDesc.toLowerCase()) >= 0) {
      if (beginsWithSkinChickenBreasts.test(direction)) {
        match = _.map(direction.match(beginsWithSkinChickenBreasts), function(str) {
          return _.trim(str);
        });
