...
var postProcessSplitOnOr = function(descriptions) {
...
var getDescriptions = parser.getDescriptions = function(text) {
  ...
  if (descriptions.length < 2) {
    ...
    descriptions = postProcessSplitOnOr(splitOnRegex(description, orSplitterRe)); // then try `or`
    if (descriptions.length > 1) {
...
var adjustDescriptionsDirections = function(quantity, measurementObj, descriptionObjs, descriptions, directions) {
  ...
  var retval = _.map(_.zip(descriptions, directions), function(tuple) { //(value, key, list)
    ...
    if (endsWithWashedRe.test(zipDesc)) {
      match = zipDesc.match(endsWithWashedRe);
      zipDesc = _.trim(zipDesc.replace(endsWithWashedRe, ''));
