...
var getDirectionsAndAlts = parser.getDirectionsAndAlts = function(text) {
  ...
  for (var i = 0, l = descriptions.length; i < l; i++) {
    ...
    if (direction) { // strip out parentheses from direction
      matches = direction.match(parenthesesGlobalRe);
      if (matches) {
        ...
        directionParentheses = matches[matches.length - 1].match(parenthesesRe);
        alt = directionParentheses[1];
        direction = direction.replace(directionParentheses[0], ''); // remove the parentheses from the direction
...
var adjustDescriptionsDirections = function(quantity, measurementObj, descriptionObjs, descriptions, directions) {
  ...
  var retval = _.map(_.zip(descriptions, directions), function(tuple) { //(value, key, list)
    ...
    if (endsWithForDrizzlingRe.test(zipDesc)) {
      match = zipDesc.match(endsWithForDrizzlingRe);
      zipDesc = _.trim(zipDesc.replace(endsWithForDrizzlingRe, ''));
    ...
    if (beginsWithCoarselyChoppedRe.test(zipDesc)) {
      ...
      zipDesc = _.trim(match[2]);//_.trim(zipDesc.replace(endsWithForDrizzlingRe, ''));
      zipDir.direction = _.trim(match[1]);
    }
