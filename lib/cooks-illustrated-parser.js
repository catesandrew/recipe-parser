...
var getDescriptions = parser.getDescriptions = function(text) {
  ...
  if (descriptions.length < 2) {
    descriptions = postProcessSplitOnOr(splitOnRegex(description, orSplitterRe)).descriptions; // then try `or`
    if (descriptions.length > 1) {
  ...
  if (parentheses && parentheses.indexOf('or') === 0) {
    descriptions.push(splitOnRegex(parentheses, orSplitterRe)[1]);
