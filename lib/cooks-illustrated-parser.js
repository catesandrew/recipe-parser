...
var _words = [
  'chopped', 'cracked', 'fresh',  'grated', 'ground', 'hard-cooked', 'minced', 'ripe', 'shredded', 'toasted'
];
...
var endsWithFromXLemondRe = /(\s+from\s+(\d+|one|two|three|four|five|six)\s+(?:lemon|lemons))$/;
  ...
  var retval = _.map(_.zip(descriptions, directions), function(tuple) { //(value, key, list)
    ...
    _.each(beginsWithList, function(beginsWith) {
      if (beginsWith.re.test(zipDesc)) {
        ...
        zipDesc = match[2];
        zipDir.direction = _.compact([match[1], zipDir.direction]).join(', ');
      }
    });
